...
QString SchemaToPrompt::getCoreTaskRules()
{
    return QString::fromUtf8(
        ...
        "7. **角色刻画**：准确描述姿态、表情、位置\n"
        "8. **对白处理**：只提取小说中明确的直接对白，只有出现引号、冒号或清晰说话格式时才填写 dialogue；叙述、心理描写、环境描写一律不要放入 dialogue，应该放入 scene / visualPrompt / narrativeFunction；如果某个面板没有明确对白，dialogue 必须为空数组。标注说话者、气泡类型、情感\n"
        "9. **叙事功能**：明确面板的故事作用\n"
