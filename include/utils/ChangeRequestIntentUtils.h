...
inline QString extractReplacementSourceFromText(const QString& text);
inline QString extractRemovalSubject(const QJsonObject& params);
inline QString extractPlacementTargetFromText(const QString& text);
...
inline bool isBackgroundEditIntentText(const QString& text)
{
...
inline bool isAttributeEditIntentText(const QString& text)
{
    const QString lower = text.trimmed().toLower();
    if (lower.isEmpty()) {
        return false;
...
inline QString buildRemovalSubjectPrompt(const QString& subject)
{
    return LocalEditPromptUtils::buildLocalRemovalEditPrompt(subject, QString(), 1.20);
}

inline QString buildReplacementSubjectPrompt(const QString& subject,
...
inline QString buildMovementSubjectPrompt(const QString& subject,
                                          const QString& destinationDescription)
{
    return LocalEditPromptUtils::buildLocalMovementEditPrompt(
        subject,
...
inline QString extractLocalObjectSubjectFromParams(const QJsonObject& params, const QString& sourcePrompt)
{
    ...
    if (subject.isEmpty()) {
        subject = extractRemovalSubject(params);
    }
    return subject;
}

inline QString extractLocalObjectDestinationFromParams(const QJsonObject& params, const QString& sourcePrompt)
{
    const QString sourceText = effectiveEditSourceText(params, sourcePrompt);
    QString destination = extractFirstTrimmedValue(params, QStringList{
        QStringLiteral("destination"),
        QStringLiteral("targetContainer"),
    ...
    if (destination.isEmpty()) {
        destination = extractPlacementTargetFromText(sourceText);
    }
    return destination;
}

inline QString extractBackgroundSubject(const QJsonObject& params)
{
    ...
    if (!timeOfDay.isEmpty()) {
        ...
        if (!translated.isEmpty()) {
            return translated + QStringLiteral("背景");
        }
        return timeOfDay + QStringLiteral("背景");
    }

    return QStringLiteral("背景");
}

inline QString buildBackgroundEditPrompt(const QJsonObject& params)
{
    return LocalEditPromptUtils::buildLocalBackgroundEditPrompt(
        QStringLiteral("背景"),
        extractBackgroundSubject(params),
        QStringLiteral("保持人物、构图、透视、镜头、光线、色调和画幅比例不变"),
        1.20);
}
...
inline QString cleanupReplacementTarget(QString text)
{
    text = text.trimmed();
    if (text.isEmpty()) {
        return QString();
    ...
    const int punctuationIndex = text.indexOf(QRegularExpression(QStringLiteral(R"([，。,.;；、\n\r])")));
    if (punctuationIndex > 0) {
        text = text.left(punctuationIndex).trimmed();
    }

    const int bracketIndex = text.indexOf(QRegularExpression(QStringLiteral(R"([（(])")));
    if (bracketIndex > 0) {
        text = text.left(bracketIndex).trimmed();
    }
...
inline int lastLocalContextMarkerIndex(const QString& text)
{
...
inline QString cleanupReplacementSource(QString text)
{
    ...
    text.remove(QRegularExpression(QStringLiteral(
        R"(^(?:把|将|请把|请将|这个|该|这张|这幅|这页|画面中的|图中的|图里|画面里|角色|人物|主体|对象|物体|物品)\s*)")));
...
inline QString cleanupPlacementTarget(QString text)
{
    text = cleanupReplacementTarget(text);
    if (text.isEmpty()) {
        return QString();
    ...
    text.remove(QRegularExpression(QStringLiteral(R"((?:里|中|内|上|下|旁边|前面|后面|左边|右边|之中|里面)$)")));
    return text.trimmed();
}

inline QString extractMatchedCapture(const QString& text, const QRegularExpression& pattern, int captureIndex)
{
...
inline QString extractMatchedSourceText(const QString& text, const QRegularExpression& pattern, int captureIndex)
{
    return cleanupReplacementSource(extractMatchedCapture(text, pattern, captureIndex));
}

inline QString extractMatchedTargetText(const QString& text, const QRegularExpression& pattern, int captureIndex)
{
...
inline QString extractReplacementSourceFromText(const QString& text)
{
    ...
    static const SourcePattern sourcePatterns[] = {
        {QRegularExpression(QStringLiteral(R"((?:把|将|请把|请将)?([^，。,.;；、\n\r]+?)(?:替换(?:为|成|改成|变成)|改为|换成|改成|变成)\s*([^，。,.;；、\n\r]+))")), 1},
        {QRegularExpression(QStringLiteral(R"(用([^，。,.;；、\n\r]+)\s*替换\s*([^，。,.;；、\n\r]+))")), 2},
        {QRegularExpression(QStringLiteral(R"((?:replace|swap)\s+([^，。,.;；、\n\r]+?)\s+(?:with|to)\s+([^，。,.;；、\n\r]+))"), QRegularExpression::CaseInsensitiveOption), 1}
    };
...
inline QString extractReplacementTargetFromText(const QString& text)
{
    const QString trimmed = text.trimmed();
    if (trimmed.isEmpty()) {
        return QString();
    ...
    static const QRegularExpression patterns[] = {
        QRegularExpression(QStringLiteral(R"(替换(?:角色|人物|主体)?为([^，。,.;；、\n\r]+))")),
        QRegularExpression(QStringLiteral(R"(替换(?:角色|人物|主体)?成([^，。,.;；、\n\r]+))")),
        QRegularExpression(QStringLiteral(R"(替换(?:角色|人物|主体)?改成([^，。,.;；、\n\r]+))")),
        QRegularExpression(QStringLiteral(R"(替换(?:角色|人物|主体)?变成([^，。,.;；、\n\r]+))")),
        QRegularExpression(QStringLiteral(R"(替换为([^，。,.;；、\n\r]+))")),
        QRegularExpression(QStringLiteral(R"(替换成([^，。,.;；、\n\r]+))")),
        QRegularExpression(QStringLiteral(R"(改为([^，。,.;；、\n\r]+))")),
        QRegularExpression(QStringLiteral(R"(换成([^，。,.;；、\n\r]+))")),
        QRegularExpression(QStringLiteral(R"(改成([^，。,.;；、\n\r]+))")),
        QRegularExpression(QStringLiteral(R"(变成([^，。,.;；、\n\r]+))")),
        QRegularExpression(QStringLiteral(R"(用([^，。,.;；、\n\r]+)替换)")),
        QRegularExpression(QStringLiteral(R"((?:replace\s+with|swap\s+to)\s*([^，。,.;；、\n\r]+))"), QRegularExpression::CaseInsensitiveOption)
    };
...
inline QString extractPlacementSourceFromText(const QString& text)
{
    const QString trimmed = text.trimmed();
    if (trimmed.isEmpty()) {
        return QString();
    ...
    static const QRegularExpression patterns[] = {
        ...
        QRegularExpression(QStringLiteral(R"((?:insert|place|put)\s+([^，。,.;；、\n\r]+?)\s+(?:into|in|on|onto|inside)\s+([^，。,.;；、\n\r]+))"), QRegularExpression::CaseInsensitiveOption)
    };
